...
type OutreachDraft={subject:string;message:string}
...
const MAX_RESEARCH_OFFER_LENGTH=6000
...
const capabilityTerms=['cto','architecture','product','ai','python','fastapi','django','flask','java','kotlin','spring','javascript','typescript','react','vue','angular','node','node.js','golang','rust','c#','.net','php','laravel','ruby','rails','swift','flutter','react native','aws','azure','gcp','kubernetes','docker','terraform','postgres','postgresql','mysql','mongodb','redis','kafka','snowflake','databricks','machine learning','artificial intelligence','data engineering','devops','backend','frontend','fullstack','mobile','payments','fintech','cybersecurity','blockchain','sales','marketing','design','product management']
function offerTerms(value:string,industry:string){
 const normalizedValue=value.toLowerCase()
 const skills=capabilityTerms.filter(term=>new RegExp(`(^|[^a-z0-9])${term.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}([^a-z0-9]|$)`,'i').test(normalizedValue))
 const industryWords=new Set(industry.toLowerCase().match(/[a-z0-9+#.-]+/g)||[])
 const fallback=(value.match(/[a-zA-Z][a-zA-Z0-9+#.-]{2,}/g)||[]).filter((term,index,all)=>{
...
function outreachPayload(value:unknown):OutreachDraft|null{
 if(typeof value==='string'){
  const clean=value.trim().replace(/^```(?:json)?\s*/i,'').replace(/\s*```$/,'')
  try{return outreachPayload(JSON.parse(clean))}catch{return null}
 }
 if(Array.isArray(value)){
  for(const item of value){const nested=outreachPayload(item);if(nested)return nested}
  return null
 }
 if(!value||typeof value!=='object')return null
 const record=value as Record<string,unknown>
 if(typeof record.message==='string'&&record.message.trim())return {subject:typeof record.subject==='string'?record.subject.trim():'',message:record.message.trim()}
 for(const key of ['output','response','content','result','text','data']){
  const nested=outreachPayload(record[key])
  if(nested)return nested
 }
 return null
}
...
function App(){
 const [cvLoading,setCvLoading]=useState(false),[cvName,setCvName]=useState(''),[screen,setScreen]=useState<Screen>(location.hash==='#history'?'history':'form'),[offer,setOffer]=useState('Python dev agency, 5 people, fintech backend specialists'),[market,setMarket]=useState('B2B SaaS startups, Series A-B, USA'),[industry,setIndustry]=useState('Fintech'),[size,setSize]=useState('11-50'),[leads,setLeads]=useState(demo),[signalCount,setSignalCount]=useState(demo.reduce((count,lead)=>count+lead.news.length,0)),[selected,setSelected]=useState(0),[msg,setMsg]=useState(0),[outreach,setOutreach]=useState(''),[contact,setContact]=useState(''),[role,setRole]=useState(''),[channel,setChannel]=useState('Email'),[subject,setSubject]=useState(''),[copied,setCopied]=useState(false),[error,setError]=useState(''),[enriching,setEnriching]=useState(false),[regenerating,setRegenerating]=useState(false),[regeneration,setRegeneration]=useState(0),[outreachError,setOutreachError]=useState(''),[pdfLoading,setPdfLoading]=useState(false),[pdfError,setPdfError]=useState('')
 ...
 async function generateOutreach(target:Lead,variation:number,contactValue=contact,roleValue=role,channelValue=channel){
  if(regenerating)return
  setRegenerating(true);setOutreachError('')
  try{
