...
function App(){
 ...
 async function search(){setError('');setMsg(0);setScreen('loading');try{let next=demo;if(API){const researchOffer=offer.trim().slice(0,MAX_RESEARCH_OFFER_LENGTH);const runResearch=async(offerQuery:string)=>{const response=await fetch(`${API}/research`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({offer:researchOffer,offer_query:offerQuery,industry,location:market,stages:[size],signals:['funding','hiring','launch']})});if(!response.ok){const body=await response.json().catch(()=>null) as {message?:string}|null;throw new Error(body?.message||`Research failed (${response.status})`)}return response.json()};let data=await runResearch(offerSearchTerms(researchOffer,industry));if(!(data.results||[]).length)data=await runResearch(industry==='Any'?'software OR technology':industry);next=fromQualification(data.qualification);if(!next.length)next=fromResults(data.results||[],researchOffer,industry)}setLeads(next);setScreen('results')}catch(e){setError(e instanceof Error?e.message:'Research failed');setScreen('form')}}
