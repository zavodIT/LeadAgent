...
function reportHtml(leads:Lead[],offer:string){
...
function App(){
 ...
 function prepareOutreach(next:Lead){setOutreach(normalizeOutreach(next.outreach));setContact('');setRole('');setChannel('Email');setSubject(`${next.name} — ${next.type==='funded'?'congrats on the recent round':'a quick idea'}`);setRegeneration(0);setRegenerating(false)}
 ...
 async function downloadPdf(){
  ...
  try{
   ...
   const response=await fetch(`${API}/pdf/generate`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({html:reportHtml(leads,offer),filename:'leadagent-report.pdf'})})
   ...
   anchor.href=url;anchor.download=data.filename||'leadagent-report.pdf';document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1000)
